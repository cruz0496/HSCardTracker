...
public enum OtherAbilitiesEnum {
    ...
    INCREMENT_ATTRIBUTE(
            "Increment Attribute", "Adds or subtracts a value from something’s Attack, Health, Cost, or Durability."),
    JOUST(
            "Joust",
            "Reveals a random card from each player’s deck. If the player who initiated "
    ...
    NO_DURABILITY_LOSS(
            "No Durability Loss", "The possessing hero’s weapon does not lose any Durability during their turn."),
    PERMANENT(
            "Permanent",
            "Permanent “minions” lack mana cost, Attack and Health, cannot be "
    ...
    PUT_INTO_BATTLEFIELD(
            "Put into Battlefield",
            "Directly places a card from the player’s deck "
    ...
    SUMMON(
            "Sumon",
            "Summons the specified minion(s). Distinct from simply playing a minion card"
    ...
    TRANSFORM_IN_HAND(
            "Transform in Hand",
            "At the start of the controlling player’s turn, "
                    + "changes a card into something else while it is in the player’s hand."),
    UNLIMITED_ATTACKS(
            "Unlimited Attacks", "Allows the possessing hero to attack an unlimited number of times each turn.");
